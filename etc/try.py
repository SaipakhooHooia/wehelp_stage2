...
async def api_attract(page: int = Query(lt = total_records, gt = 0), keyword: Optional[str] = Query(None)):
...
async def api_attractid(attractionId: int = Path(lt = total_records+1)):
	sql = """
		SELECT id, name, category, description, address, transport, mrt, lat, lng, image 
		FROM turist_spot 
		WHERE id = %s
